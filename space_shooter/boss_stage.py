...
class Boss(pygame.sprite.Sprite):
    def __init__(self, algorithm='astar'):
        ...
        # AI cho Boss (không dùng nếu muốn boss di chuyển đơn giản)
        self.ai = BossAI(algorithm=algorithm)
    ...
    def update(self, player_rect=None):
        ...
        Args:
            player_rect: Rect của người chơi
        """
        if self.use_ai and player_rect:
            try:
                ...
                if action.get('detected') and self.ai.path:
                    next_pos = self.ai.get_next_position(
                        (self.rect.centerx, self.rect.centery), speed=self.speed)
                    self.rect.centerx = int(next_pos[0])
                    self.rect.centery = int(next_pos[1])
                else:
        ...
        # Giữ boss trong vùng màn hình
        self.rect.centerx = max(self.rect.width // 2, min(self.rect.centerx, SCREEN_WIDTH - self.rect.width // 2))
        self.rect.centery = max(self.rect.height // 2, min(self.rect.centery, SCREEN_HEIGHT - self.rect.height // 2))

        if not self.use_ai or not player_rect:
            # Đổi chiều khi chạm biên chỉ ở chế độ mặc định
            if self.rect.left <= 0:
                self.rect.left = 0
...
def run_boss_stage(screen, clock, background, font_path=None, use_ai=False):
    running = True
    game_over = False
    boss_defeated = False
    ...
    boss = Boss()
    boss.use_ai = use_ai  # Thiết lập AI cho boss
    all_sprites.add(boss)

    for i in range(4):
        m = Enemy()
        m.use_ai = use_ai  # Thiết lập AI cho enemies
        all_sprites.add(m)
        enemies.add(m)
    ...
    while running:
        ...
        if not game_over:
            ...
            # Cập nhật boss với AI (truyền player_rect)
            if boss.alive():
                boss.update(player.rect)
            
            # Cập nhật enemies với AI (truyền player_rect)
            ...
            if boss.alive() and now - boss.last_shot > boss.shoot_delay:
                boss.last_shot = now
                boss_action = getattr(boss, 'current_action', {'should_shoot': True})
                if len(enemy_bullets) < 30 and boss_action.get('should_shoot', True):
